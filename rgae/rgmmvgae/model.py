...
class ReGMM_VGAE(nn.Module):
    ...
    def pretrain(self, adj, features, adj_label, weight_tensor, norm, epochs, lr, save_path, dataset):
    ...
    def update_graph(self, adj, emb, unconf_indices, conf_indices):
        k = 0
        y_pred = self.predict(emb)
        emb_unconf = emb[unconf_indices]
        adj = adj.tolil()
        if len(unconf_indices) > 0:
    ...
    def train(self, adj_norm, adj, features, norm, epochs, lr, beta1, beta2 , save_path, dataset):
        ...
        for epoch in epoch_bar:
            ...
            if epoch == 0:
                adj, adj_label, weight_tensor =  self.update_graph(adj, emb, unconflicted_ind, conflicted_ind)
